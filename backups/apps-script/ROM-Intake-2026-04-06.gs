...
const INTAKE_ACTIVE_SHEET_ID = INTAKE_PROD_SHEET_ID; // ← switch to STAGE for testing
...
function doPost(e) {
  try {
    ...
    // sheet.getLastRow() returns the last row with actual data content.
    // It does NOT count ARRAYFORMULA spill results as content.
    // Writing to lastRow+1 targets a naturally blank row that ARRAYFORMULA
    // already covers — no physical row insertion, formula stays intact.
