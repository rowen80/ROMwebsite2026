...
const DOC_TEMPLATE_NAME = "ROM_INVOICE_TEMPLATE_2026"; // TEST MODE
...
const GOOGLE_REVIEW_URL = "https://g.page/r/Cc_eRhp1RUwpEAE/review";
...
function buildReviewBlockText_() {
 return (
  "Leave a Review ⭐⭐⭐⭐⭐\n" +
  "If you are happy with our service and would like to show your appreciation, or help us improve, " +
  "please leave a review at the link below. It really helps our business grow:\n" +
  GOOGLE_REVIEW_URL
 );
}

function buildReviewBlockHtml_() {
 return (
  "<b>Leave a Review</b> ⭐⭐⭐⭐⭐<br>" +
  "If you are happy with our service and would like to show your appreciation, or help us improve, " +
  "please leave a review at the link below. It really helps our business grow:<br>" +
...
function onOpen() {
 ...
 .addSeparator()
 .addItem("Send Photos Only + Review Request (by ClientLastName)", "sendPhotosOnlyWithReviewByLastName")
 .addItem("Send Photos + Invoice + Review Request (by ClientLastName)", "sendPhotosPlusInvoiceWithReviewByLastName")
 .addItem("Send THANK YOU + Review Request (by ClientLastName)", "sendThankYouWithReviewByLastName")
 )
 .addSubMenu(
 SpreadsheetApp.getUi()
...
function sendPhotosOnlyWithReviewByLastName() {
 const lastName = prompt_("Photos Only + Review Request (by ClientLastName)", "Enter ClientLastName (case-insensitive exact match):");
 if (!lastName) return;
 runPhotosOnly_({ mode: "LASTNAME", lastNameQuery: lastName, preview: false, includeReview: true });
}
...
function sendPhotosPlusInvoiceWithReviewByLastName() {
 const lastName = prompt_("Photos + Invoice + Review Request (by ClientLastName)", "Enter ClientLastName (case-insensitive exact match):");
 if (!lastName) return;
 runPhotosPlusInvoice_({ mode: "LASTNAME", lastNameQuery: lastName, preview: false, includeReview: true });
}
...
function runPhotosOnly_(opts) {
 ...
 const linksText = buildMediaLinksText_(items, ctx.idx) + (opts.includeReview ? "\n\n" + buildReviewBlockText_() : "");
 const linksHtml = buildMediaLinksHtml_(items, ctx.idx) + (opts.includeReview ? "<br><br>" + buildReviewBlockHtml_() : "");
 ...
 `Hi ${escapeHtml_(clientName)},<br><br>` +
 "Photos are available at the link below. Please let me know if there is anything else I can do for you. " +
 "This link will be valid for 30 days. Please download the files to your personal device for permanent storage.<br><br>" +
 linksHtml +
...
function sendThankYouWithReviewByLastName() {
 const lastName = prompt_("THANK YOU + Review Request (by ClientLastName)", "Enter ClientLastName (case-insensitive exact match):");
 if (!lastName) return;
 runThankYouDownloadLink_({ mode: "LASTNAME", lastNameQuery: lastName, preview: false, includeReview: true });
}
...
function buildMediaLinksHtml_(items, idx) {
 ...
 if (video) {
 s += `<b>Video Link:</b><br>${linkOrTextHtml_(video)}<br><br>`;
 }

 if (reels) {
 s += `<b>Reels Link:</b><br>${linkOrTextHtml_(reels)}<br><br>`;
 }

 if (tour) {
 s += `<b>Tour Link:</b><br>${linkOrTextHtml_(tour)}<br><br>`;
 }
