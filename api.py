...
def create_job(job_in: JobCreate):
    ...
    try:
        ...
        job = Job(
            ...
            invoice_number=None,
            job_date=None,
            agent_first_name=clean_text(job_in.first_name),
            agent_last_name=clean_text(job_in.last_name),
...
def export_jobs_sync(request: Request, include_cancelled: int = 0):
    ...
    try:
        ...
        rows = (
            q.order_by(Job.job_date.desc().nullslast(), Job.id.desc())
            .limit(5000)
            .all()
        )
        ...
        for job, cust in rows:
            out.append({
                "job_id": job.id,
                "customer_id": cust.id,
                "first_name": job.agent_first_name or cust.first_name,
                "last_name": job.agent_last_name or cust.last_name,
